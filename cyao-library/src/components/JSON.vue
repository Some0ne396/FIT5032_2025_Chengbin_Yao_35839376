...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="data in opening_hours" >
            {{ data.key }} : {{ data.value.open }}
        </li>
      </ul>
      ...
      <p>We operate in:</p>
      <ul>
        <li v-for="data in get_bookstores.countries" >
            {{ data }}
        </li>
      </ul>
      <p>Our #1 seller:</p>
      <ul>
        <li v-for="data in get_bookstores.topSellers" >
            {{ data }}
        </li>
      </ul>
    </section>
    ...
    <section class="lab-section">
      ...
      <ul>
        <li v-for="author in authors" :key="author.id">
          <div :class="{highlight: author.id==isActive}">
             {{ author.name }} ({{ author.birthYear }})
             <button  @click="isActive = author.id"> Toggle</button>
          </div>
        </li>
      </ul>
...
const austen = computed(() => {
  ...
  let temp =   authors.filter((author) => author.id == "1")[0]
  return temp
}
)
...
const number_stores = computed(() => {
  return bookstores.totalStores
}
)

const store_type = computed(() => {
  let temp=[]
  
  for (const [key, value] of Object.entries(bookstores.storeTypes) ){
...
const opening_hours = computed(() => {
  ...
  for (const [key, value] of Object.entries(bookstores.openingHours) ){
    temp.push({"key":key,"value":value})
  }
  return temp
}
